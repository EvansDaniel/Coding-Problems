...
// TODO: WRITE A MATRIX CLASS THAT OVERLOADS THE ADDITION, SUBTRACTION,
// TODO: AND MULTIPLICATION OPERATORS
...
void strassen(matrix C, matrix A, matrix B) {
    ...
    if (A.size == 1) {
        ...
        // initialize the n/2 X n/2 matrices
        ...
        // initialize products of the submatrices
        ...
        // compute the submatrices to do the addition
        // and subtraction on s1 - s10

        // submatrix (matrix A, int x, int y, int size)
        // top left
        ...
        // addition and subtraction using above submatrices
        // to build the s1-s10 matrices as defined in clrs
        minus(s1, b12, b22);
        plus(s2, a11, a12);
        plus(s3, a21, a22);
        minus(s4, b21, b11);
        ...
        // multiplication of the 7 matrices
        strassen(p1, a11, s1);
        strassen(p2, s2, b22);
        strassen(p3, s3, b11);
        strassen(p4, a22, s4);
        ...
        // zero out the c (result) matrix
        zero(c11);
        zero(c12);
        zero(c21);
        zero(c22);

        // add and subtract the p1-p7 matrices created in the strassen
        // method calls above.
        add(c11, p5);
        add(c11, p4);
        sub(c11, p2);
